...
int SM3_256_Final(unsigned char *digest, SM3_256_CTX *ctx)
{
	if(ctx == NULL) return 0;
	unsigned int i;
...
void sm3(const unsigned char *msg, size_t  msglen, unsigned char dgst[SM3_DIGEST_LENGTH])
{
	...
	SM3_256_Init(&ctx);
	SM3_256_Update(&ctx, msg, msglen);
	// SM3_256_Final(&ctx, dgst);
	SM3_256_Final(dgst, &ctx);

	memset(&ctx, 0, sizeof(SM3_256_CTX));
}
